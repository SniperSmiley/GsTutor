...
use yew_router::routes::AppRoute;
...
pub fn editor(props: &Props) -> Html {
    ...
    let article_get = props.slug.clone().map(|slug|
                ...
                if let Some(article_get) = article_get {
                    if let Some(article_info) = &article_get.data {
                        update_info.set(ArticleCreateUpdateInfo {
                            ...
                            body: article_infoarticle.description.body.clone(),
                                                ...
                                                html! {for tag_list.iter().map(|tag| {
                                                    ...
                                                    html! {
                                                        <span class="tag-default tag-pill">
                                                            <i class="ion-close-round" onclick={onclick_remove}></i>
                                                            { &tag }
                                                        </span>
                                                    }
                                                })}
